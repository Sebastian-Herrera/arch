...
echo '0' | sudo pacman -Sy --noconfirm numlockx xdg-user-dirs neofetch noto-fonts ttf-apple-emoji ttf-liberation unzip awesome qtile picom hsetroot gtk2 gtk3 hyper-git rofi rofimoji dunst xdg-utils perl-file-mimeinfo ranger nautilus file-roller pulseaudio playerctl unrar fzf catimg redshift ntfs-3g wget gnome-keyring uget zathura zathura-pdf-mupdf vlc python-pip tldr #alacritty
...
yay -Sy --noconfirm nautilus-open-any-terminal scrot bottom glow google-chrome uget-integrator-chrome visual-studio-code-bin notion-app figma-linux spotify spicetify-cli #zsh-theme-powerlevel10k-git
...
rm -fv ~/.zlogin ~/user.sh ~/.bash* #~/.zshrc.pre-oh-my-zsh 
#startx
reboot
